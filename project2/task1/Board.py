...
class Board():
    ...
    def check_put(self, pos):
        ...
        if (self.state[pos] != '.') or (pos < 0) or (pos > 15):
            valid_flag = False
    ...
    def check_move(self, s, t, player):
        ...
        e1 = [s, t]
        e2 = [t, s]
        if self.state[s] != player.symbol:
            valid_flag = False

        if self.state[t] != '.':
            valid_flag = False

        if (e1 not in self.edges) and (e2 not in self.edges):
            valid_flag = False
    ...
    def check_remove(self, pos, player):
        ...
        opponent = '#' if player.symbol == '@' else '@'
        if self.state[pos] != opponent:
            valid_flag = False
    ...
    def form_mill(self, pos, player):
        ...
        for i in self.mills:
            for j in i:
                if pos == j:
                    x1, x2, x3 = i
                    if (self.state[x1] == player.symbol) \
                            and (self.state[x2] == player.symbol) \
                            and (self.state[x3] == player.symbol):
                        if_form = True
                    break
